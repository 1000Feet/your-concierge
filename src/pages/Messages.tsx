  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-heading font-bold">Messaggi</h1>
          <p className="text-muted-foreground">Gestisci le conversazioni</p>
        </div>
        ...
        <Card className="lg:col-span-1 flex flex-col overflow-hidden">
          <div className="p-3 space-y-2 border-b">
            <div className="relative">
              ...
              <Input placeholder="Cerca..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-sm" />
            </div>
            <div className="flex gap-1">
              {(["all", "client", "provider"] as const).map((f) => (
                <Button key={f} variant={filter === f ? "default" : "ghost"} size="sm" className="h-7 text-xs"
                  onClick={() => setFilter(f)}>
                  {f === "all" ? "Tutti" : f === "client" ? "Clienti" : "Provider"}
                </Button>
              ))}
            </div>
          </div>
          <div className="flex-1 overflow-auto">
            {filteredConvs?.map((conv) => {
              const name = conv.providers?.name ?? (conv.clients ? `${conv.clients.first_name} ${conv.clients.last_name}` : "Senza nome");
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className="text-[10px] text-muted-foreground">
                        {conv.last_message_at ? format(new Date(conv.last_message_at), "d MMM HH:mm", { locale: it }) : ""}
                      </span>
              ...
              <div className="p-3 border-b">
                <p className="font-medium text-sm">
                  {selectedConv?.providers?.name ??
                    (selectedConv?.clients ? `${selectedConv.clients.first_name} ${selectedConv.clients.last_name}` : "Conversazione")}
                </p>
