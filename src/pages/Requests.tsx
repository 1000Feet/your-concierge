...
const Requests = () => {
  ...
  const STATUS_OPTIONS: { value: RequestStatus; label: string; color: string }[] = [
    { value: "draft", label: t("status.draft"), color: "bg-muted text-muted-foreground" },
    { value: "sent", label: t("status.sent"), color: "bg-blue-100 text-blue-800" },
    { value: "waiting", label: t("status.waiting"), color: "bg-amber-100 text-amber-800" },
    { value: "confirmed", label: t("status.confirmed"), color: "bg-green-100 text-green-800" },
    { value: "completed", label: t("status.completed"), color: "bg-emerald-100 text-emerald-800" },
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: typeof form) => {
      const payload = {
        description: values.description, service_type: values.service_type, status: values.status,
        client_id: values.client_id || null, budget: values.budget ? parseFloat(values.budget) : null,
        final_price: values.final_price ? parseFloat(values.final_price) : null,
        margin: values.margin ? parseFloat(values.margin) : null,
        service_date: values.service_date || null, group_size: parseInt(values.group_size) || 1,
        notes: values.notes || null, user_id: user!.id,
      };
      if (editingRequest) {
    ...
    onSuccess: () => {
      ...
      setDialogOpen(false);
      resetForm();
      toast({ title: editingRequest ? t("requests.request_updated") : t("requests.request_created") });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["requests"] });
      toast({ title: t("requests.request_deleted") });
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-heading font-bold">{t("requests.title")}</h1>
          <p className="text-muted-foreground">{t("requests.subtitle")}</p>
        </div>
        <div className="flex gap-2">
          <ExportCSVButton
            data={(filtered ?? []).map((r: any) => ({
              ...r, client_name: r.clients ? `${r.clients.first_name} ${r.clients.last_name}` : "",
            }))}
            filename={t("requests.title").toLowerCase()}
            columns={[
              { key: "description", label: t("requests.description") },
              { key: "client_name", label: t("requests.client") },
              { key: "service_type", label: t("requests.service_type") },
              { key: "status", label: t("common.status") },
              { key: "service_date", label: t("common.date") },
              { key: "budget", label: t("common.budget") },
              { key: "final_price", label: t("requests.final_price") },
              { key: "margin", label: t("requests.margin") },
            ]}
          />
          <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
            <DialogTrigger asChild>
              <Button><Plus className="mr-2 h-4 w-4" />{t("requests.new_request")}</Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{editingRequest ? t("requests.edit_request") : t("requests.new_request")}</DialogTitle>
              </DialogHeader>
              <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(form); }} className="space-y-4">
                    ...
                    {!editingRequest && (
                      <Button type="button" variant="ghost" size="sm" className="h-6 text-xs"
                        onClick={async () => {
                          ...
                          try {
                            const { data, error } = await supabase.functions.invoke("ai-extract", { body: { text: form.description } });
                            if (error) throw error;
                            const ex = data.extracted ?? {};
                            setForm((f: typeof form) => ({
                              ...f, service_type: ex.service_type ?? f.service_type,
                            ...
                            toast({ title: t("requests.ai_extracted") });
                          } catch {
                            toast({ title: t("requests.ai_extract_error"), variant: "destructive" });
                          }
                        }}>
                        <Sparkles className="mr-1 h-3 w-3" />{t("requests.ai_extract")}
                      </Button>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label>{t("requests.service_type")}</Label>
                ...
                <div className="space-y-1">
                  ...
                  <Select value={form.client_id} onValueChange={(v) => setForm({ ...form, client_id: v })}>
                    <SelectTrigger><SelectValue placeholder={t("requests.select_client")} /></SelectTrigger>
                ...
                <Button type="submit" className="w-full" disabled={saveMutation.isPending}>
                  {editingRequest ? t("common.update") : t("common.create")}
                </Button>
              </form>
            </DialogContent>
          </Dialog>
      ...
      <div className="flex gap-3 items-center">
        <div className="relative max-w-sm flex-1">
          ...
          <Input placeholder={t("requests.search_placeholder")} value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{t("requests.service")}</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">{t("requests.no_requests")}</TableCell></TableRow>
              ) : (
                filtered?.map((r) => {
                  ...
                  return (
                    <TableRow key={r.id}>
                      ...
                      <TableCell><Badge variant="secondary">{SERVICE_TYPES.find((s) => s.value === r.service_type)?.label ?? r.service_type}</Badge></TableCell>
                      <TableCell><Badge className={statusInfo?.color ?? ""}>{statusInfo?.label ?? r.status}</Badge></TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="icon" onClick={() => navigate(`/requests/${r.id}`)}><Eye className="h-4 w-4" /></Button>
