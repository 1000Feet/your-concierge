...
const RequestDetail = () => {
  ...
  const PROVIDER_STATUS: { value: RequestProviderStatus; label: string; color: string }[] = [
    { value: "pending", label: t("provider_status.pending"), color: "bg-muted text-muted-foreground" },
    { value: "contacted", label: t("provider_status.contacted"), color: "bg-blue-100 text-blue-800" },
    { value: "accepted", label: t("provider_status.accepted"), color: "bg-green-100 text-green-800" },
    { value: "declined", label: t("provider_status.declined"), color: "bg-red-100 text-red-800" },
  ];
  ...
  const { data: availableProviders } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("providers")
        ...
        .eq("category", request!.service_type).eq("is_active", true)
  ...
  const addProviderMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("request_providers").insert({
        request_id: id!, provider_id: selectedProviderId,
    ...
    onSuccess: () => {
      ...
      setSelectedProviderId(""); setQuotedPrice(""); setProviderNotes("");
      toast({ title: t("providers.provider_assigned") });
    },
  ...
  const updateProviderStatusMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["request-providers", id] });
      toast({ title: t("requests.status_updated") });
    },
  });
  ...
  const removeProviderMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["request-providers", id] });
      toast({ title: t("providers.provider_removed") });
    },
  });
  ...
  const handleAiSuggest = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("ai-suggest", {
        body: { service_type: request.service_type, description: request.description, budget: request.budget, group_size: request.group_size },
      });
      if (error) throw error;
      setAiMessage(data.suggestion || t("requests.no_suggestion"));
    } catch (err: any) {
      toast({ title: t("messages.ai_error"), description: err.message, variant: "destructive" });
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleGenerateMessage = async (targetName: string, targetType: "provider" | "client") => {
    ...
    setGeneratedMessage(t("common.generating"));
    try {
      const { data, error } = await supabase.functions.invoke("ai-generate", {
        body: {
          type: targetType, request_description: request.description,
          service_type: request.service_type, service_date: request.service_date,
          group_size: request.group_size, target_name: targetName,
          client_name: request.clients ? `${request.clients.first_name} ${request.clients.last_name}` : undefined,
        },
      });
      if (error) throw error;
      setGeneratedMessage(data.message || t("requests.generation_error"));
    } catch {
      setGeneratedMessage(t("requests.generation_error"));
    }
  };
  ...
  if (!request) return <div className="flex items-center justify-center h-64 text-muted-foreground">{t("requests.not_found")}</div>;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-heading font-bold">{t("requests.detail")}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1">
          <CardHeader><CardTitle className="text-lg">{t("requests.info")}</CardTitle></CardHeader>
          <CardContent className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-muted-foreground">{t("requests.service")}</span>
              <Badge variant="secondary">{t(`service_types.${request.service_type}`, request.service_type)}</Badge>
            </div>
            {request.clients && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">{t("requests.client")}</span>
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">{t("requests.group")}</span>
              <span>{request.group_size ?? 1} {t("common.people")}</span>
            </div>
            {request.budget && (
              <div className="flex justify-between">
            ...
            {request.final_price && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">{t("requests.final_price")}</span>
                <span className="font-medium">€{Number(request.final_price).toFixed(0)}</span>
              </div>
            )}
            {margin != null && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">{t("requests.margin")}</span>
            ...
            {request.clients && (
              <div className="pt-2 border-t">
                <Button variant="outline" size="sm" className="w-full"
                  onClick={() => handleGenerateMessage(`${request.clients!.first_name} ${request.clients!.last_name}`, "client")}>
                  <MessageSquare className="mr-2 h-3 w-3" />{t("requests.generate_client_msg")}
                </Button>
              </div>
            )}
          </CardContent>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-lg">{t("requests.assigned_providers")}</CardTitle>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={handleAiSuggest} disabled={aiLoading}>
                <Sparkles className="mr-2 h-3 w-3" />
                {aiLoading ? t("requests.analyzing") : t("requests.ai_suggest")}
              </Button>
              <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
                <DialogTrigger asChild>
                  <Button size="sm"><Plus className="mr-2 h-3 w-3" />{t("common.add")}</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>{t("requests.assign_provider")}</DialogTitle></DialogHeader>
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <Label>{t("requests.provider")} ({t(`service_types.${request.service_type}`)})</Label>
                    ...
                    <div className="space-y-1">
                      <Label>{t("requests.quoted_price")} €</Label>
                    ...
                    <Button className="w-full" onClick={() => addProviderMutation.mutate()}
          ...
          <CardContent>
            {aiMessage && (
              <div className="mb-4 p-3 rounded-lg bg-accent/10 border border-accent/20 text-sm">
                <p className="font-medium text-accent mb-1">💡 {t("requests.ai_suggestion")}</p>
                <p className="whitespace-pre-wrap">{aiMessage}</p>
              </div>
            )}
            {!assignedProviders?.length ? (
              <p className="text-muted-foreground text-sm py-4 text-center">{t("requests.no_assigned")}</p>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t("requests.provider")}</TableHead>
      ...
      <Dialog open={messageDialogOpen} onOpenChange={setMessageDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("requests.message_for", { name: messageTarget?.name })}</DialogTitle>
          </DialogHeader>
          <Textarea value={generatedMessage} onChange={(e) => setGeneratedMessage(e.target.value)} rows={8} />
          <Button variant="outline" onClick={() => { navigator.clipboard.writeText(generatedMessage); toast({ title: t("common.copied") }); }}>
